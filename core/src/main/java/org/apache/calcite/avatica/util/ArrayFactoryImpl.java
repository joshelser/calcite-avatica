...
public class ArrayFactoryImpl implements ArrayImpl.Factory {
  ...
  private static final ArrayFactoryImpl SINGLETON = new ArrayFactoryImpl();

  private ArrayFactoryImpl() {}

  public static ArrayImpl.Factory getInstance() {
    return SINGLETON;
  }

  @Override
  public ResultSet createResultSet(AvaticaType elementType, List<Object> elements, Calendar calendar) {
    ...
    try (ListIteratorCursor cursor = new ListIteratorCursor(rows.iterator())) {
      ...
      AvaticaResultSet resultSet = new AvaticaResultSet(null, state, signature, resultSetMetaData, calendar.getTimeZone(), frame);
  ...
  @Override
  public Array createArray(AvaticaType elementType, List<Object> elements) {
    ...
    try (ListIteratorCursor cursor = new ListIteratorCursor(createRowForArrayData(elements))) {
      List<Accessor> accessor = cursor.createAccessors(types, Unsafe.localCalendar(), this);
      assert 1 == accessor.size();
      return new ArrayImpl(elements, (ArrayAccessor) accessor.get(0));
    }
  }
   ...
   */
  private List<List<Object>> createResultSetRowsForArrayData(List<Object> elements) {
    List<List<Object>> rows = new ArrayList<>(elements.size());
    for (int i = 0; i < elements.size(); i++) {
      rows.add(Arrays.asList(i + 1, elements.get(i)));
    }
    return rows;
  }
